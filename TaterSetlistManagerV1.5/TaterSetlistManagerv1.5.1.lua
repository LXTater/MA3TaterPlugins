...
-- Corrects missing macro detection; handles blank command fields
...
return function()
  ...
  local function updateSequences(nameList)
    ...
    for i = 1, #nameList - 1 do
      ...
      for _, cue in ipairs(cues or {}) do
        ...
        if cmd ~= "" then
          for _, macroName in ipairs(nameList) do
            if cmd:lower():match("macro.*" .. macroName:lower()) then
      ...
      if not matched then
        debugLog("Preparing manual assignment for sequence '%s'", current)
        local result = MessageBox({
          title = "Assign Missing Macro",
          message = string.format("Sequence: %s\nNo valid macro command found.\nEnter the macro and cue name to assign manually:", current),
          inputs = {
            {name="NextSongMacro", value="", blackFilter="", whiteFilter="", vkPlugin="", maxTextLength = ""},
            {name="CueName", value="Next Song", blackFilter="", whiteFilter="", vkPlugin="", maxTextLength = ""}
          },
          commands = {
            { value = 1, name = "Assign" },
            { value = 2, name = "Skip" },
            { value = 3, name = "Return to SetlistManager" }
          }
        })

        if result.result == 3 then
          Printf("[TaterSetlistMgr]: User chose to exit macro fixing early.")
          return
        elseif result.result == 1 and result.input and result.input["NextSongMacro"] and result.input["CueName"] then
          local cueName = result.input["CueName"]
          local macroName = result.input["NextSongMacro"]
          Cmd('Set Sequence "' .. current .. '" Cue "' .. cueName .. '" Property "Command" "Go+ Macro ' .. macroName .. '"')
          Printf("[TaterSetlistMgr]: Manually assigned cue '%s' in %s → Go+ Macro %s", cueName, current, macroName)
        end
      end
    end
  end
