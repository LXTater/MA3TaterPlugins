-- grandMA3 Plugin: TaterFX Generator (Live Updating)
-- Author: ChatGPT + Tater
-- Version: Refactor 1.1 (Live Update Enabled)
...
local function main()
  ...
  local function promptSpreadPhase()
    return MessageBox({
      ...
      message = "Set spread and phase values.",
      commands = {
        { name = "Back", value = -1 },
        { name = "Cancel", value = 0 },
        { name = "OK", value = 1 }
  ...
  -- === Apply Phaser Logic ===
  local function applyPhaser(attrList, high, low, shape, phaseX, phaseY)
    ...
    if phaseX and phaseY then
      Cmd('Set Phase ' .. tostring(phaseX) .. ' ' .. tostring(phaseY))
    end
  end
  ...
  if attribute == 2 then
    ...
    shape = shapes[posShape]
  end

  local low, high = promptValues()
  if not low or not high then return end

  -- 🔄 Live update after high/low
  applyPhaser(attrList, high, low, shape)

  local spread = promptSpreadPhase()
  if not spread or spread.result ~= 1 then return end

  local phaseX = tonumber(spread.inputs.PhaseX) or 0
  local phaseY = tonumber(spread.inputs.PhaseY) or 360

  -- 🔄 Live update after phase/spread
  applyPhaser(attrList, high, low, shape, phaseX, phaseY)
